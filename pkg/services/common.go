...
// RESTGetIntOwner handles a GET on int-pool-owner requests
...
func (service *ContrailService) GetIntOwner(
	...
	var v string
	if request.GetPool() == "" {
		err := errutil.ErrorBadRequest("Missing pool name for getting int owner")
		return nil, err
	}
	if err := service.InTransactionDoer.DoInTransaction(ctx, func(ctx context.Context) error {
		var err error
		if v, err = service.IntPoolAllocator.GetIntOwner(ctx, request.GetPool(), request.GetValue()); err != nil {
			return errutil.ErrorBadRequestf("Failed to fetch int owner: %s", err)
		}
		return nil
	}); err != nil {
		return nil, err
	}
	return &GetIntOwnerResponse{Owner: v}, nil
}
