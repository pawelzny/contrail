...
// IntOwner represents the entity which allocate int under given key.
...
func (db *Service) GetIntOwner(ctx context.Context, key string, id int64) (string, error) {
...
func (db *Service) deleteIntOwner(ctx context.Context, tx *sql.Tx, key string, id int64) error {
	d := db.Dialect
	_, err := tx.ExecContext(ctx,
